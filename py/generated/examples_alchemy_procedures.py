...
class ExamplesAlchemyProcedures:
...
"""
        return DbCmd(self.con, sql, { 'id': id, 'description': description })
...
SELECT @_return_value [return_value];
"""
        return DbCmd(self.con, sql, { 'name': name, 'description': description, 'picture': picture })
...
SELECT @_return_value [return_value];
"""
        return DbCmd(self.con, sql, { 'customerId': customerId })
...
SELECT @_return_value [return_value];
"""
        return DbCmd(self.con, sql, { 'ids': ids })
...
SELECT @_return_value [return_value];
"""
        return DbCmd(self.con, sql, { 'datetime2': datetime2, 'time': time, 'decimal': decimal })
